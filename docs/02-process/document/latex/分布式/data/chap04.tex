...
上述截图 S07 展示了微信小程序端的商品结算页面，用户可以在该页面确认购物车中选购的水果品种及数量、选择或新增收货地址、查看应付总金额并提交订单。截图 S08 展示了调用模拟支付接口后返回的支付成功 JSON 响应，其中包含操作成功标识 code 值 200 和支付流水号。截图 S04 则展示了支付服务控制台的日志输出，能够清晰看到 OpenFeign 远程调用业务服务接口的完整 HTTP 请求与响应信息，包括目标服务名、请求路径、请求方法以及返回的 200 状态码。
...
上述截图 S05 展示了 RabbitMQ 管理后台中 pay.success.queue 队列与 pay.exchange 交换机的绑定关系视图，可以看到队列已成功绑定且含有消息流转的统计数据。截图 S06 展示了浏览器开发者工具网络面板中 WebSocket 连接的详细信息，包括成功建立的 ws 协议握手以及接收到的来单提醒文本帧数据。截图 S09 展示了管理端后台页面右上角弹出的 Element Plus 全局浮动通知卡片，卡片中显示了"您有新的生鲜订单，请及时包装！"的提醒内容以及对应的订单编号。
